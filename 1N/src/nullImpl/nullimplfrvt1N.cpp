...
ReturnStatus
NullImplFRVT1N::createFaceTemplate(
    ...
    std::vector<FRVT::EyePair> &eyeCoordinates)
{
    auto templString = std::to_string(faces.size()) +
            " Somewhere out there (face), beneath the pale moon light\n";
...
FRVT::ReturnStatus
NullImplFRVT1N::createIrisTemplate(
    const std::vector<FRVT::Image> &irises,
    FRVT::TemplateRole role,
    std::vector<uint8_t> &templ,
    std::vector<FRVT::IrisAnnulus> &irisLocations)
{
    auto templString = std::to_string(irises.size()) +
            " Somewhere out there (iris), beneath the pale moon light\n";
    templ.resize(templString.size());
    memcpy(templ.data(), templString.c_str(), templString.size());
...
FRVT::ReturnStatus
NullImplFRVT1N::createFaceAndIrisTemplate(
    const std::vector<FRVT::Image> &facesIrises,
    FRVT::TemplateRole role,
    std::vector<uint8_t> &templ)
{
    auto templString = std::to_string(facesIrises.size()) +
            " Somewhere out there (face + iris), beneath the pale moon light\n";
    templ.resize(templString.size());
    memcpy(templ.data(), templString.c_str(), templString.size());
