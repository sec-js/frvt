...
FRVTLIB="libfrvt_11_twins_<company>_<three digit sequence>.so"
frvtlib_track="libfrvt_11_twins"
root=$(pwd)
approot=$root/bin
libroot=$root/lib
...
# pass into Makefile
#numLibs=$(ls $libroot/libfrvt_11_twins_*_???.so | wc -l)
numLibs=$(ls $libroot/${frvtlib_track}_*_???.so | wc -l)
...
libstring=$(ls $libroot/${frvtlib_track}_*_???.so)
