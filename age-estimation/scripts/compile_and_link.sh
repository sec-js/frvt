...
FRVTAELIB="libfrvt_ae_<company>_<three digit sequence>.so"
root=$(pwd)
approot=$root/bin
libroot=$root/lib
...
# pass into Makefile
numLibs=$(ls $libroot/libfrvt_ae_*_???.so | wc -l)
if [ $numLibs -eq 0 ]; then
	echo "[ERROR] Could not find core implementation library in $libroot.  Please make sure your core implementation library is named according to the required naming convention - $FRVTAELIB and placed in $libroot."
	exit $failure
elif [ $numLibs -gt 1 ]; then
	echo "[ERROR] There are more than one libraries in $libroot that matches the core implementation library naming convention - $FRVTAELIB.  There should only be ONE core submission library."
	exit $failure
fi

libstring=$(ls $libroot/libfrvt_ae_*_???.so)
